...
function TrainerCard({ trainer }: { trainer: Trainer }) {
  ...
  return (
    <div className="bg-white rounded-xl border p-4 hover:border-purple-400 hover:shadow-sm transition-all flex items-center gap-4">
      <button
      ...
      >
        <Pin
          size={16}
          className={trainer.isPinned ? "text-purple-600 fill-purple-600" : "text-gray-300"}
        />
      </button>
...
export default function TrainerList({ trainers }: { trainers: Trainer[] }) {
  ...
  return (
    <div className="space-y-6">
      {pinned.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Festede trenere</p>
      ...
      <div>
        {pinned.length > 0 && (
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Alle trenere</p>
        )}
        <div className="space-y-3">
