...
export default async function TrainerBookPage({ params }: { params: Promise<{ trainerId: string }> }) {
  ...
  const { data: trainer } = await supabase
    .from("profiles")
    .select("id, name, phone, avatar_url, club_id, clubs(name)")
    .eq("id", trainerId)
    .eq("role", "trainer")
    .single();
  ...
  return (
    <main className="bg-gray-50 dark:bg-gray-950 px-6 pb-6 page-safe-top">
      <div className="max-w-lg mx-auto">
        <Link href="/book" className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-gray-100 dark:bg-gray-800 hover:bg-[#E2A9F1]/20 text-gray-700 dark:text-gray-200 mb-2 -ml-2">
