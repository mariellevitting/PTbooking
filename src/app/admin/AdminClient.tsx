...
import { formatWeekday, formatMonth, formatTime as formatTimeLocale } from "@/lib/dateUtils";
...
const ROLE_COLORS = {
  dancer: { bg: "bg-blue-100 dark:bg-blue-900", text: "text-blue-700 dark:text-blue-300" },
  parent: { bg: "bg-green-100 dark:bg-green-900", text: "text-green-700 dark:text-green-300" },
  trainer: { bg: "bg-orange-100 dark:bg-orange-900", text: "text-orange-700 dark:text-orange-300" },
};

function timeAgo(t: ReturnType<typeof useTranslations>, dateStr: string) {
...
function TrainerCalendar({ trainer, slots }: { trainer: Profile; slots: Slot[] }) {
  ...
  const DAYS = Array.from({ length: 7 }, (_, i) => formatWeekday(new Date(2024, 0, 1 + i), locale, "short"));
  ...
  const weekLabel = t("weekLabel", {
    ...
    startMonth: formatMonth(weekStart, locale, "short"),
    endDay: weekEnd.getDate(),
    endMonth: formatMonth(weekEnd, locale, "short"),
  });
  ...
  const listView = (
    <div className="md:hidden space-y-2">
        ...
        return (
          <div key={i}>
            <p className={`text-xs font-bold mb-1 ${isToday ? "text-[#E2A9F1]" : "text-gray-500 dark:text-gray-400"}`}>
              {DAYS[i]} {day.getDate()}. {formatMonth(day, locale, "short")}
            </p>
              ...
              {daySlots.map(s => {
                ...
                return booking ? (
                  <div key={s.id} className={`flex items-center gap-2 rounded-lg px-3 py-2 border ${isPast ? "bg-gray-50 dark:bg-gray-800/40 border-gray-200 dark:border-gray-700 opacity-50" : "bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800"}`}>
                    ...
                    <span className="text-xs font-semibold text-gray-700 dark:text-gray-300 w-10 shrink-0">{formatTimeLocale(new Date(s.start_at), locale)}</span>
                    <span className="text-xs text-gray-600 dark:text-gray-400 truncate flex-1">{booking.dancer_name} · {booking.dance_style}</span>
                    {isPast && <span className="text-[10px] text-gray-400 shrink-0">{t("completed")}</span>}
                  </div>
                ) : (
                  <div key={s.id} className={`flex items-center gap-2 rounded-lg px-3 py-2 border ${isPast ? "opacity-30 bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700" : "bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700"}`}>
                    ...
                    <span className="text-xs font-semibold text-gray-500 w-10 shrink-0">{formatTimeLocale(new Date(s.start_at), locale)}</span>
                    <span className="text-xs text-gray-400">{isPast ? t("notBooked") : t("free")}</span>
                  </div>
                );
              })}
            </div>
      ...
      {weekDays.every(day => trainerSlots.filter(s => isSameDay(new Date(s.start_at), day)).length === 0) && (
        <p className="text-xs text-gray-400 text-center py-2">{t("noLessonsThisWeek")}</p>
      )}
    </div>
  );
  ...
  const gridView = (
    <div className="hidden md:block">
                ...
                {daySlots.map(s => {
                  ...
                  return booking ? (
                    <div key={s.id} className={`rounded-md px-1 py-0.5 ${isPast ? "bg-gray-300 dark:bg-gray-600 opacity-50" : "bg-green-500 text-white"}`}>
                      <p className="text-[10px] font-semibold leading-tight">{formatTimeLocale(new Date(s.start_at), locale)}</p>
                    ...
                    <div key={s.id} className={`rounded-md px-1 py-0.5 ${isPast ? "opacity-25 bg-gray-200 dark:bg-gray-700" : "bg-gray-200 dark:bg-gray-700"}`}>
                      <p className="text-[10px] text-gray-500 dark:text-gray-400 leading-tight">{formatTimeLocale(new Date(s.start_at), locale)}</p>
                      <p className="text-[10px] text-gray-400 leading-tight">{t("free")}</p>
                    </div>
                  );
                })}
              </div>
      ...
      <div className="flex gap-4 mt-3">
        <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-green-500" /><span className="text-xs text-gray-500">{t("upcoming")}</span></div>
        <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-gray-300 dark:bg-gray-600" /><span className="text-xs text-gray-500">{t("completed")}</span></div>
        <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-gray-200 dark:bg-gray-700" /><span className="text-xs text-gray-500">{t("free")}</span></div>
      </div>
    </div>
  );
...
export default function AdminClient({ profiles: initialProfiles, feedback, slots, trainerMap, isAdmin, clubs }: Props) {
  ...
  async function createClub() {
    ...
    const body = await res.json().catch(() => ({ error: t("clubs.unknownError") }));
    setNcSaving(false);
    if (res.ok && body.id) {
      window.location.href = `/admin/klubb/${body.id}`;
    } else {
      setNcError(body.error ?? t("clubs.createFailed"));
    }
  }
  ...
  async function sendBroadcast() {
    ...
    if (!confirm(t("broadcast.confirmSend", { count: ids.length }))) return;
    setSending(true);
    setSentInfo("");
    ...
    setSending(false);
    setSentInfo(t("broadcast.sentInfo", { count: ids.length }));
    setMsgTitle("");
    setMsgBody("");
  }

  async function handleDeleteUser(id: string, name: string) {
    if (!confirm(t("userList.confirmDelete", { name }))) return;
    setDeletingId(id);
    ...
    if (res.ok) {
      ...
      const { error } = await res.json().catch(() => ({ error: t("clubs.unknownError") }));
      alert(t("userList.deleteFailed", { error }));
    }
  }
  ...
  const statCards = [
    { label: t("stats.dancers"), value: dancers.length, filter: "dancer" as RoleFilter, color: "bg-blue-500 border-blue-500" },
    { label: t("stats.parents"), value: parents.length, filter: "parent" as RoleFilter, color: "bg-green-500 border-green-500" },
    { label: t("stats.trainers"), value: trainers.length, filter: "trainer" as RoleFilter, color: "bg-orange-500 border-orange-500" },
  ];
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{t("subheading", { count: profiles.length })}</p>
          </div>
        </div>
        ...
        {isAdmin && <div className="bg-white dark:bg-gray-900 rounded-2xl border dark:border-gray-700 p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-gray-900 dark:text-white">{t("clubs.heading")}</h2>
            <button onClick={() => setNewClubOpen(o => !o)} className="text-xs font-semibold text-[#9b59c4] dark:text-[#E2A9F1] hover:underline">
              {newClubOpen ? t("clubs.cancel") : t("clubs.newClub")}
            </button>
          </div>

          {newClubOpen && (
            <div className="border dark:border-gray-700 rounded-xl p-4 mb-3 space-y-2 bg-[#f5eeff] dark:bg-[#E2A9F1]/5">
              <input value={ncName} onChange={e => setNcName(e.target.value)} placeholder={t("clubs.namePlaceholder")} className="w-full border dark:border-gray-700 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-white" />
              <input value={ncShort} onChange={e => setNcShort(e.target.value)} placeholder={t("clubs.shortNamePlaceholder")} className="w-full border dark:border-gray-700 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-white" />
              ...
              <button onClick={createClub} disabled={ncSaving || !ncName.trim()}
                ...
                {ncSaving ? t("clubs.creating") : t("clubs.create")}
              </button>
              <p className="text-xs text-gray-400">{t("clubs.hint")}</p>
            </div>
          )}
                ...
                <div key={club.id} className="border dark:border-gray-700 rounded-xl p-4">
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-3">
                    <div className="bg-orange-50 dark:bg-orange-900/20 rounded-lg p-2 text-center">
                      ...
                      <p className="text-xs text-gray-500">{t("stats.trainers")}</p>
                    </div>
                    <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-2 text-center">
                      <p className="text-lg font-bold text-blue-600">{dancersInClub.length}</p>
                      <p className="text-xs text-gray-500">{t("stats.dancers")}</p>
                    </div>
                    <div className="bg-green-50 dark:bg-green-900/20 rounded-lg p-2 text-center">
                      <p className="text-lg font-bold text-green-600">{parentsInClub.length}</p>
                      <p className="text-xs text-gray-500">{t("stats.parents")}</p>
                    </div>
                  </div>
                  <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 space-y-1.5">
                    <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">{t("clubs.codesHeading")}</p>
                    {club.trainer_code && <div className="flex justify-between text-sm"><span className="text-gray-500">{t("clubs.codeTrainer")}</span><span className="font-mono font-bold text-gray-900 dark:text-white">{club.trainer_code}</span></div>}
                    {club.dancer_code && <div className="flex justify-between text-sm"><span className="text-gray-500">{t("clubs.codeDancer")}</span><span className="font-mono font-bold text-gray-900 dark:text-white">{club.dancer_code}</span></div>}
                    {club.parent_code && <div className="flex justify-between text-sm"><span className="text-gray-500">{t("clubs.codeParent")}</span><span className="font-mono font-bold text-gray-900 dark:text-white">{club.parent_code}</span></div>}
                    {!club.trainer_code && !club.dancer_code && !club.parent_code && (
                      <p className="text-xs text-gray-400">{t("clubs.noCodesHint")}</p>
                    )}
                  </div>
                </div>
              );
        ...
        {isAdmin && (
          <div className="bg-white dark:bg-gray-900 rounded-2xl border dark:border-gray-700 p-5">
            <h2 className="font-bold text-gray-900 dark:text-white mb-1">{t("broadcast.heading")}</h2>
            <p className="text-xs text-gray-400 mb-4">{t("broadcast.subheading")}</p>
            <div className="grid grid-cols-2 gap-3 mb-3">
              <select value={msgRole} onChange={e => setMsgRole(e.target.value as any)} className="border dark:border-gray-700 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-white">
                <option value="all">{t("broadcast.allRoles")}</option>
                <option value="trainer">{t("broadcast.trainers")}</option>
                <option value="dancer">{t("broadcast.dancers")}</option>
                <option value="parent">{t("broadcast.parents")}</option>
              </select>
              <select value={msgClub} onChange={e => setMsgClub(e.target.value)} className="border dark:border-gray-700 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-white">
                <option value="all">{t("broadcast.allClubs")}</option>
                {clubs.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            </div>
            <input value={msgTitle} onChange={e => setMsgTitle(e.target.value)} placeholder={t("broadcast.titlePlaceholder")} className="w-full border dark:border-gray-700 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-white mb-2" />
            <textarea value={msgBody} onChange={e => setMsgBody(e.target.value)} rows={3} placeholder={t("broadcast.bodyPlaceholder")} className="w-full border dark:border-gray-700 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-white mb-3" />
            <div className="flex items-center gap-3">
              <button onClick={sendBroadcast} disabled={sending || !msgBody.trim() || msgRecipients.length === 0}
                ...
                {sending ? t("broadcast.sending") : t("broadcast.send", { count: msgRecipients.length })}
              </button>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border dark:border-gray-700 p-5">
          <h2 className="font-bold text-gray-900 dark:text-white mb-4">{t("trainerOverview.heading")}</h2>
          <div className="space-y-3">
                  ...
                  <button onClick={() => setTrainerOpen(isOpen ? null : trainer.id)}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-gray-400">{t("trainerOverview.stats", { bookedCount, availableCount })}{isAdmin && trainer.club_id ? ` · ${clubs.find(c => c.id === trainer.club_id)?.name ?? ""}` : ""}</p>
                      </div>
                    </div>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border dark:border-gray-700 p-5">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-bold text-gray-900 dark:text-white">
                {roleFilter === "alle" ? t("userList.allUsers") : roleFilter === "dancer" ? t("stats.dancers") : roleFilter === "parent" ? t("stats.parents") : t("stats.trainers")}
                <span className="ml-2 text-gray-400 font-normal">{filteredProfiles.length}</span>
              </h2>
              <button onClick={() => setRoleFilter(null)} className="text-xs text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">{t("userList.close")}</button>
            </div>
            <div className="space-y-1">
                  ...
                  <div key={p.id} className="flex items-center justify-between py-2.5 border-b dark:border-gray-700 last:border-0">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {isAdmin && p.club_id && <p className="text-xs text-gray-400">{clubs.find(c => c.id === p.club_id)?.name ?? t("userList.unknownClub")}</p>}
                      </div>
                    </div>
                    <div className="flex items-center gap-3 shrink-0 ml-2">
                      <span className="text-xs text-gray-400">{timeAgo(t, p.created_at)}</span>
                      {isAdmin && (
                        <button
                        ...
                        >
                          {deletingId === p.id ? "…" : t("userList.delete")}
                        </button>
                      )}
                    </div>
                  </div>
        ...
        {isAdmin && (
          <div className="bg-white dark:bg-gray-900 rounded-2xl border dark:border-gray-700 p-5">
            <h2 className="font-bold text-gray-900 dark:text-white mb-4">
              {t("feedback.heading")} <span className="text-[#E2A9F1] ml-1">{feedback.length}</span>
            </h2>
            {!feedback.length ? (
                  ...
                  return (
                    <div key={f.id} className="border dark:border-gray-700 rounded-xl p-3">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs text-gray-400">{timeAgo(t, f.created_at)}</span>
                      </div>
                      {colors && <span className={`text-xs ${colors.text} ${colors.bg} px-2 py-0.5 rounded-full`}>{t(`roleLabel.${f.role}` as any)}</span>}
